	...
	public partial class Viewer : DarkForm
	{
			...
			for (var i = 0; i < 10; i++)
			{
				...
				mainToolStrip.Items.Insert(after, new ToolStripButton(
					i.ToString(), null, (s, e) =>
					{
						...
						Scene.RunEvent("set|{0}", Scene.Set);
		...
		private void Screen_MouseMove(object sender, MouseEventArgs e)
		{
			...
			if (held != null)
			{
				...
				if (held.Fix > 0)
				{
					if (Tools.Distance(fix, held.Position) > FixSnap)
					{
						...
						if (held.Fix == 0)
						{
							if (!Scene.RunEvent(string.Format("unfix|{0}", HilightedCel)))
								Scene.RunEvent(string.Format("unfix|{0}", held));
						}
			...
			if (lastCelPointed != cel)
			{
				...
				{
					if (Scene.RunEvent("mouseout|{0}", lastCelPointed.ID))
					{
						DrawScene();
					}
					else
					{
						if (Scene.RunEvent("mouseout|{0}", lastCelPointed.Part.ID))
							DrawScene();
					}
				}
				if (cel != null)
				{
					if (Scene.RunEvent("mousein|{0}", cel.ID))
					{
						DrawScene();
					}
					else
					{
						if (Scene.RunEvent("mousein|{0}", part.ID))
							DrawScene();
					}
				}
				lastCelPointed = cel;
		...
		private void Screen_MouseUp(object sender, MouseEventArgs e)
		{
			...
			if (held != null && held.Fix > 0)
			{
				...
				if (held.Fix == 0)
				{
					if (!Scene.RunEvent("unfix|{0}", HilightedCel))
					{
						Scene.RunEvent("unfix|{0}", held);
					}
				}
			}
...
#region Dark mode lightshit I mean switch
		...
		private void NextPal_Click(object sender, EventArgs e)
		{
			...
			Scene.RunEvent("col|{0}", Scene.Palette);
			DrawScene();
